...
Recent incidents in the United States, such as George Floyd's death, have drawn international attention to the country's racial inequalities. This type of racism has greatly contributed to the racial disparity in a variety of different areas, including financial services. Additionally, a mortgage loan is the most popular type of refinancing. Possessing a home is associated with accumulating more wealth. However, historically, people of color have been denied such mortgages at a higher rate. Thus, The goal of this study is to conduct an empirical analysis of loan approval status on different races in the United States of America. The primary objective of this project is to determine how much people of color are less likely to get approved a mortgage loan applications than white considering the same income and loan amounts. To address this topic, I regress the probability of loan approval on different races. Additional confounding variables such as gender, income, loan amount, and collateral are included in the model to support the study. Variables are described as following: 

- Dependent variable: Loan approved is binary variable. It is 1 if loan is approved, 0 otherwise
- Explanatory variable: Race: this variable contains information about races which are as following : White, Black, Asian, Hawaiian and Alaskan natives. For each of them I created a binary variable. 
...
The source of data is Bureau of Consumer Projection in the United States. The selected data set is published under Home Mortgage Disclosure Act, HMDA, section which can be found [here](https://www.consumerfinance.gov/data-research/hmda/historic-data/?geo=nj&records=all-records&field_descriptions=labels). To analyze racial inequality in mortgage lending, I selected New Jersey state. The mentioned state is accounted for one of the diverse states containing 50% White, 21% Hispanic or Latino and 12% Black or African American population and it is one of the highly diverse states based on U.S. News [_report_](https://www.usnews.com/news/best-states/slideshows/the-10-most-racially-diverse-states-in-the-us?slide=6). The downloaded financial data for 2017 is the most recent year in the website. The original data set contains 349563 rows and and 78 columns, which can be found as a zip file in this Github [Repository](https://github.com/ghazalayobi/DA2_Coding1_Final_Project/blob/main/raw/hmda_2017_nj_all-records_labels.csv.zip). HMDA is a complicated data for the purpose of this project, these columns are selected for further analysis: applicant id, loan purpose, loan amount, actions taken for the loan application, applicant race and gender, loan amount, and lien status.  
Based on [ _MPA_](https://www.mpamag.com/us/mortgage-industry/guides/the-7-most-popular-types-of-mortgage-loans-for-home-buyers/255499) the most common loan is Fixed-rate mortgage or conventional home loan. Around 90% of home buyers choose it. Thus, in this project I only consider Conventional mortgage. While working with this dataset the major problem was its size. As the HMDA data is really big, I could not view it on GitHub repository. Thus, I cleaned the data and uploaded on the mentioned repository. 

### Filters
Information not provided by the applicant about their gender and race is removed from the data. Moreover, applicants were filtered if they withdrew their loan application. Mortgage loan is the most common loan among lower and middle class families in the United States, and based on [Rutgers University](https://bloustein.rutgers.edu/are-you-middle-class-depends-where-you-live-in-nj/) in the New Jersey the middle class income is below USD 180,000. Thus, in this research I will be focusing on the American lower and middle class loan applicants. Moreover, as part of data cleaning all NA values have been dropped. 

### Dummmy Variables
I created dummy variable if loan is originated called loan_approved equal to 1 and 0 otherwise. In addition, I created more dummy variables for each racial group such as: White, Black, Asian, and Hawaiian - Alaskan natives. I added more binary variables for gender, and created another column called _Male_ which is equal to one if the applicant is male and zero if the applicant is a female. The data set also include other valuable information about about loan purpose and lien status for the loan application. I created two more dummy variables, first is _home loan_ if the loan purpose is home purchase or home improvement then it is one, zero if the loan purpose is refinancing. Collateral plays a vital role in securing a loan, thus, I created a dummy variable called 
_collateral_ which is equal to one if loan applicant is secured by first lien or subordinate lien  and zero otherwise. 

### Tranformation
Loan amount and applicant income were multiplied by 1000. As both of the variables are right skewed, their logarithms are considered for further analysis. Figure 1 and Figure 2 exhibits log distribution of both variables. As a result the sum of observations is `r sum(!is.na(df$action))`. 

# Summary Statisitcs
The summary statistics table shows that 70% of loans were approved. 80% of loan applicants are white, men are more than half of mortgage applicants. 91% of loan applications are secured by lien. 
...
Table 2 in the Appendix illustrates the probability of loan approval. Column one indicates that we can be 95% that if someone is white they are 6.8% more likely to get a loan and Asians are 3.09% more likely to get it. However, Black or African American, Hawaiian and Alaskan are 18% less likely to get approval for a loan application. For further illustrations, the scatter plots and regression lines that correspond to each regression is shown in the Appendix figures : 4, 5, 6, and 7. However, the scatter plots for each regression line is in corners because both the dependent variable and explanatory variables are dummy variables. Thus, observations can have those four combinations. The size of the dots are proportional to their frequency in the data. Figure 4 and 6 show that Probability of loan approval and white, same as Asian, have a positive slope, on the contrary, Figure 5 and 7 have negative slope indicating a probability of loan approval and black, same as hawaiian or Alaskan,  are negatively correlated. To get closer to uncover the effects of racial disparities, loan approval is regressed on all races considering _Asian_ as a base category. 

## Model 1
$$LoanApproved^P=\alpha+\beta_1( hawaiian-alaskan )+\beta_2( black ) +\beta_3( white )$$

To address the problem of predicting probabilities that are less than zero or greater than one, there are two models as alternatives to the linear probability model which are logit and probit models. Pseudo R-squared is used to evaluate goodness of fit of a logistic and probit models. It is similar to the R-squared which measures how much better the fit is compared to what it would be if we were not using any of the right-hand-side variables for prediction. Table 3 shows the result of Pseudo R-squared for Model 1 for both logit and probit regressions which is 1%. 


## Model 2

To further evaluate the model, I added two control variables which are _collateral_ and _log of income_. Correlation Matrix in Appendix, Figure 3, showed that there is positive correlation between Probability of loan approval, log of income and collateral. As it can be seen from the Table 3 that Pseudo R-squared changes from 1% to 4.7% or both logistic and probit regression of Model 2.
